    ...
    fn gen_req_tags(&self, tags: Vec<&str>) {
        ...
        for &tag in tags.iter() {
            let (req_tag, module_link, suffix) = if tag.contains("::") {
                ...
                let suffix = format!(" of the {} module", create_link(&module_link, module_name));
        ...
        re.replace_all(input, |caps: &regex::Captures| {
            ...
            if tag.starts_with("http://") || tag.starts_with("https://") {
                ...
                if let Some(module_name) = parts.first() {
                    ...
                    create_link(&format!("{}#{}", module_link, spec_tag), text)
                } else {
                    create_link(&format!("#t{}", tag), text)
                }
            } else {
                create_link(&format!("#{}", tag), text)
            }
        })
        .to_string()
    }
...
fn create_link(anchor: &str, label: &str) -> String {
    format!("[{}]({})", anchor, label)
}
